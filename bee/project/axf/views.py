...
def home(request):
    ...
    return render(request, 'axf/home.html',
                  {"title": "主页", "wheelsList": wheelsList,
...
def market(request, categoryid, cid, sortid):
    ...
    return render(request, 'axf/market.html',
                  {"title": "闪送超市", "leftSlider": leftSlider,
...
def changecart(request, flag):
    ...
    if flag == '0':
        ...
        product.storenums -= 1
        product.save()
        return JsonResponse({"data": c.productnum, "price": c.productprice,
                             "status": "success"})
    elif flag == '1':
        carts = Cart.objects.filter(userAccount=user.userAccount)
        c = None
    ...
    if request.method == "POST":
        ...
        if f.is_valid():
            ...
            return render(request, 'axf/login.html',
                          {"title": "登陆", "form": f, "error": f.errors})
    else:
        f = LoginForm()
        return render(request, 'axf/login.html',
                      {"title": "登陆", "form": f})
...
def register(request):
    if request.method == "POST":
        ...
        user = User.createuser(userAccount, userPasswd, userName,
                               userPhone, userAdderss, userImg,
                               userRank, userToken)
        user.save()
